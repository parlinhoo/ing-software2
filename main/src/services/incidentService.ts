...
// Claves de estado en español: el backend (PUT /incident/:id/estado) las espera así.
...
export type IncidentAPI = {
  ...
  incidentType: string
  severity: string
  actors: Array<{ name: string; rut?: string; role: string }>
  date: string
  place: string
  description: string
...
export async function fetchIncidents(options?: { mine?: boolean }): Promise<IncidentAPI[]> {
  // mine=true le pide al backend solo los incidentes registrados por el usuario actual.
  // Backend ignora el flag para roles no-Docente (Inspector ya viene filtrado siempre).
  const url = options?.mine ? `${Paths.INCIDENT}?mine=true` : Paths.INCIDENT
  const response = await axiosInstance.get<IncidentAPI[]>(url)
...
export async function searchStudents(query: string): Promise<StudentData[]> {
    // RUT: formato XXXXXXXX-X  |  ALUMNO: string para buscar por nombre
...
export async function registerIncident(
    ...
    const data = { registerer, incidentType, severity, actors, date, place, description }
...
export async function editIncident(
    ...
    const data = { incidentId, incidentType, severity, actors, date, place, description }
    print(`Petición de editado de incidente ${incidentId} enviada...`);
    const response = await axiosInstance.put(`${Paths.INCIDENT}${incidentId}`, data);
...
export async function deleteIncident(incidentId: number, motivo: string) {
    print(`Petición de eliminado de incidente ${incidentId} enviada...`);
    const response = await axiosInstance.delete(`${Paths.INCIDENT}${incidentId}`, { data: { motivo } });
    if (response.status === HttpStatusCodes.OK) {
...
// CU-11 / US-12: cambia el estado del incidente. Llama al endpoint real PUT /incident/:id/estado.
...
/* ───────────────── INTERVENCIONES / SEGUIMIENTO (US-12, US-13) ───────────────── */
...
// US-12: registrar acción de seguimiento. POST /intervention (persistencia en BD, valida rol Orientador).
...
// Listar acciones de seguimiento de un incidente (orden cronológico inverso, ya lo ordena el backend).
...
// US-13: editar acción de seguimiento. PUT /intervention.
...
// Eliminar acción de seguimiento (soft delete en backend). DELETE /intervention?id=...
